...
zenn_emoji: "📟"
zenn_type: "tech"
zenn_topics: ["wtfutil", "terminal", "tools", "cli", "tui"]
zenn_published: true
---
...
初回起動時に以下のメッセージが出てくると思います。

> Go to the following link in your browser then type the authorization code:
> https://accounts.google.com/o/oauth2/auth?access_type=... (press 'return' before inserting the code)

ここに表示されたURLにアクセスしAPIの使用を許可してください。**press 'return'**と書いてありますが、まだリターンキーは押さないように
APIを有効にすると`http://localhost/...`へリダイレクトしようとします。しかし、このリダイレクトは失敗するのでリダイレクトしようとしたURLをコピーして適当なエディタに貼り付けましょう。

> http://localhost/?state=state-token&iss=https://accounts.google.com&code=<authorization code>&scope=https://www.googleapis.com/auth/calendar.readonly

`<authorization code>`と書かれた部分にはランダムな文字列が入っています。この文字列をコピーし、先ほどのプロンプトに入力して'return'キーを押すと認証が完了します。

これで再度wtfを起動するとGoogleカレンダーの予定がダッシュボードに表示されるでしょう。
